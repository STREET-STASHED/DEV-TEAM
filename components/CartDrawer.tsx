...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <CartContext.Provider
      value={{ items, totalCount, totalPrice, addItem, updateQuantity, removeItem, clearCart, isOpen, toggleCart }}
    >
      {children}
    </CartContext.Provider>
  );
...
const CartDrawer: React.FC<CartDrawerProps> = ({ onClose }) => {
  const cart = useCart();
  const { items, totalCount, totalPrice, updateQuantity, removeItem, clearCart, isOpen, toggleCart } = cart;
  ...
  return (
    <>
      ...
      >
        <button className="self-end mb-4 text-gray-700 hover:text-gray-900" onClick={onClose || toggleCart}>Close</button>
        <h2 className="text-xl font-semibold mb-4">Your Cart ({totalCount} items)</h2>
