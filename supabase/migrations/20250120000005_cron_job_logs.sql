...
CREATE POLICY IF NOT EXISTS cron_job_logs_select_authenticated ON cron_job_logs FOR
SELECT USING (auth.role() = 'authenticated');
-- Only authenticated users can insert logs
CREATE POLICY IF NOT EXISTS cron_job_logs_insert_authenticated ON cron_job_logs FOR
INSERT WITH CHECK (auth.role() = 'authenticated');
-- Function to clean old logs (keep last 30 days)
CREATE OR REPLACE FUNCTION clean_old_cron_logs() RETURNS INTEGER AS $$
...
SELECT cjl.job_name,
    ...
    COUNT(*) FILTER (
        WHERE cjl.success
    )::INTEGER as successful_executions,
    COUNT(*) FILTER (
        WHERE NOT cjl.success
...
WHERE cjl.executed_at >= NOW() - (days_back || ' days')::INTERVAL
    AND (
...
-- INSERT INTO cron_job_logs (job_name, orders_processed, orders_assigned, drivers_available, success, details)
-- VALUES (
--   'auto_assign_orders',
--   5,
--   3,
