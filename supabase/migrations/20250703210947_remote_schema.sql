...
for select
to authenticated
using ((bucket_id = 'verification-docs'::text));
...
for insert
to public
with check (((bucket_id = 'verification-docs'::text) AND (auth.role() = 'authenticated'::text)));
...
create policy "Upload to any folder in verification-docs"
on "storage"."objects"
as permissive
for insert
to authenticated
with check ((bucket_id = 'verification-docs'::text));


create policy "Upload to own folder (safe)"
on "storage"."objects"
as permissive
for insert
to authenticated
with check (((bucket_id = 'verification-docs'::text) AND (name ~~ (auth.uid() || '/%'::text))));


create policy "User can only upload into their own folder"
on "storage"."objects"
as permissive
for insert
to authenticated
with check ((name ~~ (auth.uid() || '/%'::text)));
