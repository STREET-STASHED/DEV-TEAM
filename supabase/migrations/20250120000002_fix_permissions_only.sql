...
DO $$ BEGIN -- Grant permissions to reviews table if it exists
IF EXISTS (
  SELECT 1
...
IF EXISTS (
  SELECT 1
  FROM information_schema.tables
  WHERE table_name = 'referral_leaderboard'
) THEN
GRANT ALL ON public.referral_leaderboard TO authenticated;
END IF;
...
DO $$ BEGIN -- Grant execute on refresh_referral_leaderboard function if it exists
IF EXISTS (
  SELECT 1
  FROM information_schema.routines
  WHERE routine_name = 'refresh_referral_leaderboard'
) THEN
GRANT EXECUTE ON FUNCTION public.refresh_referral_leaderboard() TO authenticated;
END IF;
-- Grant execute on get_average_rating function if it exists
IF EXISTS (
  SELECT 1
  FROM information_schema.routines
  WHERE routine_name = 'get_average_rating'
) THEN
GRANT EXECUTE ON FUNCTION public.get_average_rating(text, uuid) TO authenticated;
END IF;
-- Grant execute on get_review_count function if it exists
IF EXISTS (
  SELECT 1
  FROM information_schema.routines
  WHERE routine_name = 'get_review_count'
) THEN
GRANT EXECUTE ON FUNCTION public.get_review_count(text, uuid) TO authenticated;
END IF;
END $$;
-- 4. Verify the fixes
