...
WHERE delivery_fee IS NULL OR driver_pay IS NULL OR platform_margin IS NULL;
...
CREATE OR REPLACE FUNCTION get_orders_ready_for_drivers()
RETURNS TABLE(
  order_id UUID,
  buyer_name TEXT,
  seller_name TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    o.created_at
  FROM orders o
  LEFT JOIN profiles b ON o.buyer_id = b.user_id
  LEFT JOIN profiles s ON o.seller_id = s.user_id
