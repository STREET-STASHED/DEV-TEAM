...
CREATE TABLE IF NOT EXISTS public.driver_profiles (
  ...
  driver_tier TEXT DEFAULT 'Bronze' CHECK (
    driver_tier IN ('Bronze', 'Silver', 'Gold', 'Diamond')
  ),
...
CREATE TABLE IF NOT EXISTS public.orders (
  ...
  driver_id UUID REFERENCES public.driver_profiles(id) ON DELETE
  SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_driver_profiles_current_order_id ON public.driver_profiles(current_order_id);
...
CREATE TRIGGER update_driver_profiles_updated_at BEFORE
UPDATE ON public.driver_profiles FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_driver_stats_updated_at BEFORE
UPDATE ON public.driver_stats FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE POLICY "Drivers can view own profile" ON public.driver_profiles FOR
SELECT USING (auth.uid() = user_id);
CREATE POLICY "Drivers can update own profile" ON public.driver_profiles FOR
UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Drivers can insert own profile" ON public.driver_profiles FOR
INSERT WITH CHECK (auth.uid() = user_id);
-- Admins can view all driver profiles
CREATE POLICY "Admins can view all driver profiles" ON public.driver_profiles FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM auth.users
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Drivers can view own stats" ON public.driver_stats;
DROP POLICY IF EXISTS "Admins can view all driver stats" ON public.driver_stats;
-- Drivers can view their own stats
CREATE POLICY "Drivers can view own stats" ON public.driver_stats FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM public.driver_profiles
      WHERE id = driver_stats.driver_id
        AND user_id = auth.uid()
    )
  );
-- Admins can view all driver stats
CREATE POLICY "Admins can view all driver stats" ON public.driver_stats FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM auth.users
...
CREATE POLICY "Buyers can view own orders" ON public.orders FOR
SELECT USING (auth.uid() = buyer_id);
-- Sellers can view orders assigned to them
CREATE POLICY "Sellers can view assigned orders" ON public.orders FOR
SELECT USING (auth.uid() = seller_id);
-- Drivers can view orders assigned to them
CREATE POLICY "Drivers can view assigned orders" ON public.orders FOR
SELECT USING (
    driver_id IS NOT NULL
    AND EXISTS (
      SELECT 1
      FROM public.driver_profiles
      WHERE id = orders.driver_id
        AND user_id = auth.uid()
    )
  );
-- Drivers can view available orders (pending without driver)
CREATE POLICY "Drivers can view available orders" ON public.orders FOR
SELECT USING (
    status = 'pending'
    AND driver_id IS NULL
  );
-- Buyers can insert their own orders
CREATE POLICY "Buyers can insert own orders" ON public.orders FOR
INSERT WITH CHECK (auth.uid() = buyer_id);
-- Drivers can update orders assigned to them
CREATE POLICY "Drivers can update assigned orders" ON public.orders FOR
UPDATE USING (
    driver_id IS NOT NULL
    AND EXISTS (
      SELECT 1
