...
COMMENT ON COLUMN orders.distance_miles IS 'Delivery distance in miles';
COMMENT ON COLUMN orders.eta_minutes IS 'Estimated delivery time in minutes';
COMMENT ON COLUMN orders.driver_payout_amount IS 'Amount paid to driver for delivery';
COMMENT ON COLUMN orders.delivery_fee_amount IS 'Total delivery fee paid by buyer';
COMMENT ON COLUMN orders.platform_margin_amount IS 'Platform margin from delivery fee';
COMMENT ON COLUMN orders.support_fee_buyer IS 'Support fee paid by buyer';
COMMENT ON COLUMN orders.support_fee_seller IS 'Support fee paid by seller';

-- Create indexes for performance
CREATE INDEX IF NOT EXISTS idx_orders_distance_miles ON orders(distance_miles);
CREATE INDEX IF NOT EXISTS idx_orders_driver_payout ON orders(driver_payout_amount);
CREATE INDEX IF NOT EXISTS idx_orders_delivery_fee ON orders(delivery_fee_amount);
...
-- Create trigger
DROP TRIGGER IF EXISTS trigger_update_order_fees ON orders;
