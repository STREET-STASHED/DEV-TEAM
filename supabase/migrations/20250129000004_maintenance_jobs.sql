-- Automated Maintenance Jobs Migration
-- Sets up scheduled jobs for materialized view refresh and partition management
...
BEGIN
  ...
  WHILE current_month <= future_month LOOP
    BEGIN
      PERFORM public.create_monthly_partitions(current_month);
...
BEGIN
  RETURN QUERY SELECT 
    ...
    (SELECT MAX(refresh_time) FROM public.refresh_materialized_views_safe())::TIMESTAMPTZ,
  ...
  SELECT 
    ...
    (SELECT MAX(refresh_time) FROM public.manage_partitions_automatically())::TIMESTAMPTZ,
...
-- RLS policy for maintenance logs (admin only)
CREATE POLICY "Admin can view maintenance logs" ON public.maintenance_logs
FOR SELECT USING (auth.uid() IN (
  SELECT id FROM auth.users WHERE email LIKE '%@streetstashed.com'
));
...
-- Log the completion
SELECT 'Maintenance jobs setup completed successfully' as status;
