...
export default function AIStylistPage() {
  ...
  const startAnalysis = async () => {
    ...
    // Save profile before analysis and navigate to the analysis route
    saveProfile(styleProfile)

    try {
      // brief delay for UX before navigating
      await new Promise(resolve => setTimeout(resolve, 300))

      // Navigate to dedicated analysis page which will perform the work
      // Note: this file lives at `/ai-stylist/page.tsx`, so the analysis
      // route is `/ai-stylist/analysis` (not `/analysis`). Use the full path.
      router.push('/ai-stylist/analysis')
      return
    } catch (error) {
      console.error('Style analysis navigation failed:', error)
  ...
  // Pick up recommendations generated by the analysis route (if any)
  useEffect(() => {
    if (!isClient) return
    try {
      const saved = localStorage.getItem('ai-stylist-recommendations')
      if (saved) {
        ...
        if (parsed && parsed.length > 0) {
          ...
          localStorage.removeItem('ai-stylist-recommendations')
        }
      }
    } catch (e) {
      console.error('Failed to load recommendations from analysis route:', e)
    }
  }, [isClient])
